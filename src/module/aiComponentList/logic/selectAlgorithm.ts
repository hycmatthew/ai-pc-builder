import { MappedCPUType, MappedGPUType, MappedMotherboardType, MappedRAMType } from "../constant/mappedObjectTypes";
    ...
    for (const mb of compatibleMBs) {
      ...
      for (const ram of compatibleRams) {
        ...
        if (remainingBudget >= 0) {
          const bestGPU = findBestGPU(sortedGPUs, gpuLookupTable, remainingBudget);
          if (bestGPU) {
            const currentScore = calculatePerformanceScore(
              cpu.score,
              bestGPU.score,
