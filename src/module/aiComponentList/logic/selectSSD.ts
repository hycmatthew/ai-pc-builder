...
export const selectBestSSD = (ssds: MappedSSDType[]): MappedSSDType | null => {
  if (ssds.length === 0) return null

  // 优先处理推荐列表中有有效价格的SSD
  const suggestedSSD = BuildConfig.SSDFactor.SSDSuggestion
  const validSuggested = ssds.filter((ssd) => suggestedSSD.includes(ssd.name))
  ...
  const scoredSSDs = candidates.map((ssd) => {
    ...
    // 归一化处理（0-1范围）
    const normScore =
      scoreRange !== 0 ? (ssd.score - minScore) / scoreRange : 0.5
    const normPrice =
      priceRange !== 0 ? (maxPrice - ssd.price) / priceRange : 0.5
