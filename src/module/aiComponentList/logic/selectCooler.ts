...
export const selectBestCooler = (
  ...
  const suggestedCoolerIds = BuildConfig.CPUCoolerFactor.AirCoolerSuggestion

  // 2. 從候選池篩選推薦散熱器並按價格排序
  const recommendedCoolers = coolerPool
    .filter(
      (cooler) => suggestedCoolerIds.includes(cooler.id) && cooler.price !== 0
    )
    .sort((a, b) => a.price - b.price)

  console.log(recommendedCoolers)
